...
#image = ImageOps.fit(source_tmp, image_size, method=Image.LANCZOS, bleed=0.0, centering=(0.5, 0.5))
...
def loadDefaultImagesSource():
    ...
    if not default_file_exist:
        ...
        tmp = ImageOps.fit(in_file, maximum_image_size, method=Image.LANCZOS, bleed=0.0, centering=(0.5, 0.5))
    ...
    if list_tag == 'styret':
        ...
        for member in list_of_members:
            ...
            if name in cv.boardmember_img_src:
                picture = ImageTk.PhotoImage(ImageOps.fit(cv.boardmember_img_src[name], image_size, method=Image.LANCZOS, bleed=0.0, centering=(0.5, 0.5)))
                cv.boardmember_img_fit[i] = picture
                i += 1
            else:
                idx = random.randint(1,len(cv.default_boardmember_img_src))
                picture = ImageTk.PhotoImage(ImageOps.fit(cv.default_boardmember_img_src[idx], image_size, method=Image.LANCZOS, bleed=0.0, centering=(0.5, 0.5)))
                cv.boardmember_img_fit[i] = picture
                i += 1
